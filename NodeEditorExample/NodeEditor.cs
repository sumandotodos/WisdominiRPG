...
public class NodeEditor : EditorWindow {
	...
	[MenuItem("Window/Super kakkou-ii Node Editor")]
	...
	void OnGUI() {
		...
		if(e.button == 1 && !makeTransitionMode) {
			...
			if(clickedOnWindow) {
				...
				baseInputNode nodeToCange = windows[selectIndex].ClickedOnInput(mousePos);
		...
		if(makeTransitionMode && selectednode != null) {
			...
			Rect mouseRec = new Rect(e.mousePosition.x, e.mousePosition.y, 10, 10);
	...
	void ContextCallback(object obj) {
		...
		if(clb.Equals("inputNode")) {
			...
			CalcNode outputNode = new CalcNode();
